...
var Crawler = function(host, initialPath, initialPort, interval) {
    ...
    crawler.maxDetpthSoftCheck = false;
...
Crawler.prototype.softDepthAllowed = function (queueItem) {
    var crawler = this;

    return (crawler.maxDepth === 0) || (queueItem.depth <= crawler.maxDepth);
}
...
Crawler.prototype.depthAllowed = function(queueItem) {
    ...
    mimeTypesWhitelist = mimeTypesWhitelist.concat(supportedMimeTypes);
...
Crawler.prototype.queueLinkedItems = function(resourceData, queueItem, decompressed) {
    ...
    //If we need only html, don't download resources at all
    if (crawler.maxDepth !== 0 && queueItem.depth == crawler.maxDepth && crawler.maxDetpthSoftCheck) {
        return crawler;
    }
